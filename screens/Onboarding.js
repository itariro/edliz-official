...
class Onboarding extends React.Component {
  ...
  componentDidMount() {
    ...
    let didUpdateCheck = false; let latestVersion = ''; let latestVersionURL = '';
    ...
    fetch('https://padendere.co.zw/edlizadmin/api/v1/index.php?version')
      ...
      .then(responseJson => {
        ...
        latestVersion = responseJson.version[0].db_version;
        latestVersionURL = responseJson.version[0].db_data_json;
        didUpdateCheck = true;
        this.state = {
          latest_db_version: latestVersion,
          update_progress: 0,
    ...
    if (didUpdateCheck) {
      console.log(didUpdateCheck);

      // get process update
      if (latestVersion != '' && (this.state.db_version != latestVersion)) {
        ...
        console.log('could not get updates online, will use the local one');
      }
    } else {
      console.log('will use the local one');
    }
  }
